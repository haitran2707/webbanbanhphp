...
if ($result->num_rows == 0) {
    echo "<script>alert('Giỏ hàng không tồn tại!'); window.location.href='manage_market.php';</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['confirm'])) {
        $deleteQuery = "DELETE FROM giohang WHERE MaGH = ?";
        $stmt = $connect->prepare($deleteQuery);
        $stmt->bind_param("s", $maGH);

        if ($stmt->execute()) {
            echo "<script>alert('Xóa giỏ hàng thành công!'); window.location.href='manage_market.php';</script>";
            exit();
        } else {
            echo "<script>alert('Lỗi khi xóa giỏ hàng!'); window.history.back();</script>";
        }
    } else {
...
<body>
    <div class="container">
        <h2>Xác nhận xóa giỏ hàng</h2>
        <div class="user-details">
            <p><strong>Mã Giỏ Hàng:</strong> <?= htmlspecialchars($row['MaGH']) ?></p>
        </div>
        <form method="POST">
