...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['confirm'])) {
        $deleteQuery = "DELETE FROM danhmuc WHERE MaDM = ?";
        $stmt = $connect->prepare($deleteQuery);
        $stmt->bind_param("s", $maDM);

        if ($stmt->execute()) {
            echo "<script>alert('Xóa danh mục thành công!'); window.location.href='manage_categories.php';</script>";
            exit();
        } else {
            echo "<script>alert('Lỗi khi xóa danh mục!'); window.history.back();</script>";
        }
    } else {
...
<body>
    <div class="container">
        ...
        <div class="user-details">
            <p><strong>Mã Danh Mục:</strong> <?= htmlspecialchars($row['MaDM']) ?></p>
            <p><strong>Tên Danh Mục:</strong> <?= htmlspecialchars($row['TenDM']) ?></p>
        </div>
        <form method="POST">
