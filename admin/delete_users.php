...
// Kiểm tra nếu có id của users cần xóa
...
$query = "SELECT * FROM nguoidung WHERE MaND = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['confirm'])) {
        $deleteQuery = "DELETE FROM nguoidung WHERE MaND = ?";
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Lỗi khi xóa người dùng!'); window.history.back();</script>";
        }
    } else {
...
<body>
    <div class="container">
        ...
        <div class="user-details">
            <p><strong>Mã Người Dùng:</strong> <?= htmlspecialchars($row['MaND']) ?></p>
        </div>
        <form method="POST">
